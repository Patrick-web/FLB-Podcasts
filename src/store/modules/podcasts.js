...
const actions = {
  ...
  showCurrentlyPlayingPodcast({ commit }) {
    ...
    state.subscribed.forEach((pod) => {
      if (pod.podId === state.currentlyPlayingEpisode.podId) {
        state.currentlyPlayingPodcast.isSubscribed = true;
      }
    });
  ...
  unSubscribeToPodcast({ commit }, id) {
    ...
    state.renderedPodcasts.forEach((genre) => {
      genre.podcasts.forEach((renderedPod) => {
        if (renderedPod.id == id) {
          renderedPod["isSubscribed"] = false;
        }
      });
    });
    if (state.currentlyPlayingEpisode.podcast.podId == id) {
  ...
  async fetchPodcastData({ commit }, podId, nextEpPubDate) {
    document.body.classList.add("fetchingInProgress");
    document.body.classList.add("showPodcastData");
    if (hasRssFeed(podId)) {
      console.log("Already has RSS");
      const podcastWithRss = JSON.parse(localStorage.getItem(`${podId}`));
      await fetchEpisodesFromRSS(podcastWithRss.rssFeed)
        .then((data) => {
          const episodes = data;
          episodes.forEach((episode) => {
            episode.formattedDuration = timeFormatter(episode.duration);
            if (episode.enclosure) {
              episode.audio = episode.enclosure.url;
              episode.audio_length_sec = parseInt(episode.duration);
              episode.podcast = {
                name: podcastWithRss.title,
                podId: podcastWithRss.podId,
                thumbnail: podcastWithRss.thumbnail,
                isSubscribed: podcastWithRss.isSubscribed,
              };
            }
          });
          console.log(podcastWithRss);
          podcastWithRss.episodes = episodes;
          state.currentPodcast = podcastWithRss;
          document.body.classList.remove("fetchingInProgress");
        })
        .catch((e) => {
          console.log(e);
          ...
          document.body.classList.remove("fetchingInProgress");
          actions.listenNotesFetch(podId, nextEpPubDate);
        });
    } else {
      actions.listenNotesFetch(podId, nextEpPubDate);
    }
  },
  listenNotesFetch(podId, nextEpPubDate) {
    var config = {
      method: "get",
      url: `https://listen-api.listennotes.com/api/v2/podcasts/${podId}?${nextEpPubDate}&sort=recent_first`,
      headers: {
        "X-ListenAPI-Key": "ebda0a8f7b964787bb9853b6433656f2",
      },
    };
    ...
    axios(config)
      .then(function(response) {
        ...
        if (podcast.website) {
          document.querySelector("webview").src = podcast.website;
          registerRSSfeed(podcast);
        }
  ...
  setDownloadedEpisodes({ commit }, episodes) {
...
function hasRssFeed(podId) {
  if (localStorage.getItem(`${podId}`)) {
    return true;
  }
  return false;
}

function registerRSSfeed(podcast) {
  const webview = document.querySelector("webview");
  webview.addEventListener("dom-ready", () => {
    webview
      .executeJavaScript(
  ...
  let extractRssLink = (html) => {
    const $ = cheerio.load(html);
    const rgx = /rss/i;
    $("a").each((i, element) => {
      if (rgx.test($(element).text())) {
        console.log("Rss Feed Found, Registering it");
        const podcastData = {
          podId: podcast.podId,
          ...
          isSubscribed: false,
          rssFeed: $(element).attr("href"),
        };
        localStorage.setItem(`${podcast.podId}`, JSON.stringify(podcastData));
        console.log("RSS has been Registered");
      }
    });
  };
}
async function fetchEpisodesFromRSS(url) {
  const rssData = await podcastParser.getPodcastFromURL(url);
